...
# 导入数据及数据预处理
setwd("原始数据与代码对应/微生物代谢物相关性热图.代码/")
rows <- read.csv("sdm29.csv")
cols <- read.csv("lefse.genus15.csv")
str(rows)
...
# 构建相关关系矩阵
library(psych)
data.corr <- corr.test(rows[, 2:30], cols[, 2:16], method="spearman", adjust="fdr")
